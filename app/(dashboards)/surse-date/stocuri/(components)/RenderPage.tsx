...
export default function RenderPage() {
  ...
  const load = async (triggerSyncIfEmpty = false) => {
    ...
    // First visit with no data → trigger SmartBill sync automatically
            ...
            <div>
              ...
              <CardDescription className="text-xs">
                {syncing
                  ...
                  : data?.syncedAt
                    ? `Ultimul sync: ${new Date(data.syncedAt).toLocaleString("ro-RO")} · Valoare totală: ${fmtRON(totalValue)}`
                    : "Sincronizare automată zilnică la 08:00"}
              </CardDescription>
            </div>
        ...
        <CardContent className="p-0">
          {loading || syncing ? (
            ...
            <div className="h-32 flex flex-col items-center justify-center gap-2 text-sm text-muted-foreground">
              ...
              {filter ? "Niciun produs găsit." : "Niciun produs disponibil. Sincronizare automată zilnică la 08:00."}
            </div>
          ) : (
